...
import numpy as np
from functions import bandpassFilter, exportAVI, dataCursor2D
...
QSQ = np.empty_like(IB, dtype=complex)
for i in range(NY):
    for j in range(NX):
        QSQ[i, j] = ((LAMBDA*MPP)/(max([NY, NX])*N))**2*((i-NY/2)**2+(j-NX/2)**2)
#%%
...
THISE = RES
R2 = np.empty([NX, NY, Z.shape[0]], dtype=complex)

for j in range(NZ-1):
    Hqz = np.exp(IKAPPA * Z[j] - GAMMA * abs(Z[j]))
    R2[:, :, j] = np.exp(IKAPPA * Z[j] - GAMMA * abs(Z[j]))
    THISE = E * Hqz
